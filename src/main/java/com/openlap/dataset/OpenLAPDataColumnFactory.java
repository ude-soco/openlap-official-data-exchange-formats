...
/**
 * This Factory should be used to create the OpenLAPDataColumns on OpenLAPDataSets. IT holds a
 * method that accepts ID, type, required or not, title, description and returns a generated
 * OpenLAPDataColumn.
 */
    ...
    switch (type) {
      case Text:
        return new OpenLAPDataColumn<String>(
            id, OpenLAPColumnDataType.Text, isRequired, title, description);
      case Numeric:
        return new OpenLAPDataColumn<Float>(
            id, OpenLAPColumnDataType.Numeric, isRequired, title, description);
      case TrueFalse:
        return new OpenLAPDataColumn<Boolean>(
            id, OpenLAPColumnDataType.TrueFalse, isRequired, title, description);
      default:
