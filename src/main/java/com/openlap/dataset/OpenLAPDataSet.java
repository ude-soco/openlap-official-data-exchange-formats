 ...
 * main purposes: 1. Enable easy incorporated serialization into JSON with human-readable form 2.
 * Enable the dynamic validation of types and required fields as well as enable the check of valid
 * configurations, i.e. mappings between different OpenLAPDataSets to determine they are compatible
 * on their column Types and required parameters.
 */
public class OpenLAPDataSet {
  // Map of the columns with the String ID. The string is taken from the OpenLAPColumnConfigData of
  // the column.
  ...
  /**
   * Adds a column to this OpenLAPDataSet. Should be added with the help of the
  ...
  /**
   * This method realizes the need for the OpenLAPDataSet to be able to dynamically (on runtime)
   * check whether a configuration (of the OpenLAPPortConfig) is compatible with the current
   * DataSet. It checks that the types and required fields are present and that the incoming fields
   * are all part of the present OpenLAPDataSet.
   *
   * @param configuration The configuration to be checked.
   * @return A OpenLAPDataSetConfigValidationResult that contains information about the validity of
   *     the configuration and additional information about what fields are problematic in case the
   *     validation does not yield a positive result.
   */
  public OpenLAPDataSetConfigValidationResult validateConfiguration(
      OpenLAPPortConfig configuration) {
    ...
    for (OpenLAPPortMapping mappingEntry : configuration.getMapping()) {
      ...
      if (!inputColumn.validateConfigurationDataTypeFromOutputPort(mappingEntry.getOutputPort())) {
        ...
        configResult.appendValidationMessage(
            String.format(
                "Port %s expected %s, got %s instead.",
                inputColumn.getId(),
                inputColumn.getType(),
                mappingEntry.getOutputPort().getType()));
      }
    }
   ...
   *
   * @param onlyRequiredColumnsConfigurationData if true, returns only required columns
  ...
  public List<OpenLAPColumnConfigData> getColumnsConfigurationData(
      boolean onlyRequiredColumnsConfigurationData) {
    List<OpenLAPDataColumn> columns;
    if (onlyRequiredColumnsConfigurationData) columns = getColumnsAsList(true);
  ...
  /**
   * Method compares this OpenLAPDataSet against the one passed through in the argument of this
   * function
   *
   * @param openLAPDataSetToCompare The Dataset to compare against
   * @return true if both the column of this dataset are an exact match to that provided as an
   *     argument ot the function, false otherwise
   */
  public boolean compareToOpenLAPDataSet(OpenLAPDataSet openLAPDataSetToCompare) {
    List<OpenLAPColumnConfigData> columnsFirstInstance =
        new ArrayList<>(this.getColumnsConfigurationData());
    List<OpenLAPColumnConfigData> columnsSecondInstance =
        new ArrayList<>(openLAPDataSetToCompare.getColumnsConfigurationData());

    if (columnsFirstInstance.size() != columnsSecondInstance.size()) return false;
    ...
    if (columnsSecondInstance.size() == 0) return true;
    else return false;
  }
   ...
   */
  private void validatePresenceRequiredColumns(
      OpenLAPDataSetConfigValidationResult configResult, List<OpenLAPColumnConfigData> values) {
   ...
   */
  private void validateInputColumnsCorrespondence(
      OpenLAPDataSetConfigValidationResult configResult, List<OpenLAPColumnConfigData> values) {
    ...
    if (valuesCopy.size() > 0) {
      ...
      for (OpenLAPColumnConfigData remainingColumn : valuesCopy) {
        configResult.appendValidationMessage(
            String.format(
                "Column: %s does not exist in the destination dataset", remainingColumn.getId()));
      }
    }
    // Otherwise the required fields are present
    else configResult.setValid(true);
   ...
   * @param removalList
   */
  private void removeMatchingColumnData(
      List<OpenLAPColumnConfigData> original, List<OpenLAPColumnConfigData> removalList) {
